...
## Load Data
```{r}
data.survey <- read.csv("edgelist_project.csv", header=TRUE, row.names = NULL)

datasurveynew <- data.survey %>%
  group_by(Motivation, Impact) %>%
  tally(sort=TRUE) %>%
  rename(weight=n) %>% 
  ungroup()
...
#since global and local cc are 0, there is no clustering in the network. it may be because the network is small or very sparse
```
